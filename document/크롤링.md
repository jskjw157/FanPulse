...
| 콘서트 일정 데이터 (crawled_concerts)  | H007 (콘서트 일정), H015 (상세 공연 정보 페이지)    | 콘서트 일정 화면에 공연 목록 표시, 클릭 시 상세 정보 표시               |
| 아티스트 정보                          | H003 (팬 커뮤니티), H014 (아티스트 상세 페이지)     | 아티스트별 커뮤니티 구성, 상세 프로필 및 활동 정보 제공                 |
| 실시간 투표 데이터                     | H004 (투표 페이지)                                  | 외부 투표 결과를 크롤링하여 실시간 투표 현황 반영                       |
| K-POP 굿즈 및 상품                     | H008 (광고 참여), H016 (마이페이지)                 | 광고 시청 후 포인트 적립, 굿즈 구매 연결                                |
...
flowchart LR
    ...
    CrawledConcerts[crawled_concerts] -->|일정목록| H007[콘서트일정]
    ...
    ProductData[상품데이터] -->|광고| H008[광고참여]
```
