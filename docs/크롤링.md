...
| 라이브 메타데이터 (streaming_events) | 수시 또는 1시간마다 | 유동적 | YouTube/V LIVE 스트림 상태 모니터링 |
...
| 라이브 스트리밍 메타데이터 (streaming_events) | H006 (라이브 & 이벤트), H019 (라이브 상세)           | YouTube Live, V LIVE 등 외부 스트리밍 URL 및 메타데이터만 수집/관리 |
...
-   **DB 구조** (데이터베이스 정의서 기준):
    -   뉴스 (crawled_news): {title, url, content, source, published_at, created_at}
