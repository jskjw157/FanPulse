...
2. `gh pr diff {PR번호}` 실행하여 diff 조회
3. diff 내용만 리뷰 (diff에 없는 파일은 절대 리뷰하지 않음)
4. 리뷰 결과를 PR 코멘트로 게시
5. 커밋 상태를 설정하여 머지 차단/허용 결정
...
# 또는 API:
gh api repos/ohchaeeun/FanPulse/rulesets/12174915 \
  --method PUT \
  -f 'rules[0].type=required_status_checks' \
  -f 'rules[0].parameters.required_status_checks[0].context=claude-code-review'
```
...
1. **allowedTools 사전 설정** (권장):
   `.claude/settings.json`에 `gh pr diff`, `gh pr comment`, `gh api` 허용
...
| `TELEGRAM_CHAT_ID` | 봇과의 채팅 ID | 메시지 대상 지정 |
