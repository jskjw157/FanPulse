...
### Phase 1: Telegram 봇 설정 (준비)
- [ ] BotFather에서 Telegram 봇 생성
- [ ] 봇 토큰 획득
- [ ] Claude Code에 Telegram 플러그인 설치 (`/plugin install telegram@claude-plugins-official`)
- [ ] `/telegram:configure <token>` 으로 토큰 설정
- [ ] Claude Code 재시작 (`claude --channels plugin:telegram@claude-plugins-official`)
- [ ] 봇에 메시지 전송 → 페어링 코드 → `/telegram:access pair <code>`
- [ ] 접근 정책 설정 (`/telegram:access policy allowlist`)

#### Quality Gate
```bash
# Telegram 채널 연결 확인
# Claude Code 터미널에서 Telegram 메시지 수신 테스트
# 봇에 "hello" 전송 → Claude Code 세션에 <channel source="telegram"> 이벤트 도착
```

### Phase 2: GitHub Action 워크플로우 교체
- [ ] 기존 `.github/workflows/ai-code-review.yml` 백업
- [ ] 경량 워크플로우 작성 (PR 정보 → Telegram 전송)
- [ ] GitHub Secrets 추가: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`
- [ ] 리뷰 완료 감지 로직 구현 (커밋 상태 polling 또는 webhook)
...
- [ ] Claude Code instructions 설정 (Telegram 채널용 리뷰 프롬프트)
- [ ] `gh pr diff` → 리뷰 → `gh pr comment` 파이프라인
- [ ] `gh api repos/{owner}/{repo}/statuses/{sha}` 로 커밋 상태 설정
...
- Bun 런타임 (Telegram 플러그인 의존)
- `gh` CLI 인증 완료 (로컬)
- Claude Code Telegram 플러그인 (`claude-plugins-official`)
