...
> **범위(MVP)**: 본 문서의 Milestone(크롤링 파이프라인 구축)은 `뉴스/차트/콘서트/광고` 데이터 적재를 우선 대상으로 합니다.
...
| 라이브 메타데이터 (streaming_events) | 수시 또는 1시간마다 | 유동적 | YouTube/Weverse Live 스트림 상태 모니터링 |
...
| 라이브 스트리밍 메타데이터 (streaming_events) | H006 (라이브 & 이벤트), H019 (라이브 상세)           | YouTube Live, Weverse Live 등 외부 스트리밍 URL 및 메타데이터만 수집/관리 |
...
flowchart LR
    ...
    ExternalStream[외부스트리밍<br>YouTube/Weverse Live] -->|메타데이터| H006
...
| 버전  | 날짜       | 변경 내용                                                                                    |
| ----- | ---------- | ------------------------------------------------------------------------------------------------ |
| 1.0.0 | 2024-12-20 | 크롤링 기획서 초안                                                                            |
| 1.1.0 | 2025-12-28 | 크롤링 스케줄 구체화<br>- 데이터 종류별 실행 빈도 및 시간대 명시<br>- H006/H007 화면 구분 명확화<br>- 아티스트 정보 소스 우선순위 정의 (Melon > Spotify > Last.fm)<br>- crawled_ads 테이블 스키마 확정 |
